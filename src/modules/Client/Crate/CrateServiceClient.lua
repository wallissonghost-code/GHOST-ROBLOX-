...
function S:Init()
	self._maid=Maid.new()
	self._remoteFunctionPromise=self._maid:Add(PromiseGetRemoteFunction("CrateServiceRemoteFunction"))
	self._model=ObservableMap.new()
	self._maid:GiveTask(self._model)
end

function S:Start()
	self._maid:GiveTask(RxInstanceUtils.observeLastNamedChildBrio(Players.LocalPlayer,"Folder","RelicInventory"):Pipe({
		...
		maid:GiveTask(RxValueBaseUtils.observeValue(obj):Subscribe(function(count)
			self._model:Set(obj.Name,count)
		end))
		maid:GiveTask(function()self._model:Remove(obj.Name)end)
	end))
end

function S:GetModel()return self._model end
function S:PromiseTryUnbox()
	return self._remoteFunctionPromise:Then(function(remote)
		return RemoteFunctionUtils.promiseInvokeServer(remote,"Unbox")
	end)
end
