...
function CrateButton:_render()
 local props={Text="ROLL",TextColor3=WxTheme.TEXT,TextSize=18,Size=UDim2.fromScale(1,1),BackgroundTransparency=1,FontFace=Font.fromId(WxTheme.ID_SANS,Enum.FontWeight.Bold)}
 local desired=TextServiceUtils.observeSizeForLabelProps(props):Pipe({Rx.map(function(s)return Vector3.new(s.X+54,s.Y+22) end),Rx.defaultsTo(Vector3.new(0,40))})
 ...
 return Blend.New("ImageButton")({ClipsDescendants=true,Image=WxTheme.PATTERN_STRIPE,BackgroundColor3=WxTheme.PRIMARY_A,ImageColor3=WxTheme.PRIMARY_B,ScaleType=Enum.ScaleType.Tile,TileSize=UDim2.fromOffset(32,32),Visible=spring:Pipe({Rx.map(function(s)return s.X.Offset>8 end)}),Size=spring,Blend.New("UICorner")({CornerRadius=UDim.new(0,8)}),Blend.New("UIStroke")({Thickness=2,Color=WxTheme.TEXT}),Blend.New("TextLabel")(props)})
end
return CrateButton
